...
on DSP physics features, with MLflow experiment tracking and ONNX export.
...
N_FEATURES = 39  # Exact count of DSP features from extract_features_dsp.py
...
def prepare_features(
    ...
    """
    Prepare feature matrix and labels from DSP features DataFrame.

    Features = [embedding_0..38, pipe_material_encoded, pressure_bar]
              = 41 dimensions total (39 DSP features + 2 metadata)
    ...
    Returns:
        X             : (n_samples, 41) float32 feature matrix
        y             : (n_samples,) int labels
        embeddings    : (n_samples, 39) float32 — used for Isolation Forest
    ...
    if mean_var < 1e-3:
        ...
        print("Your embeddings are mathematically identical across all audio clips.")
        print("If you are using YAMNet, it has collapsed the leaks into generic static.")
        print("XGBoost is failing because it has no numerical variance to split on.")
        print("FIX: You MUST run extract_features_dsp.py to generate physical features.")
        print("!" * 70 + "\n")
