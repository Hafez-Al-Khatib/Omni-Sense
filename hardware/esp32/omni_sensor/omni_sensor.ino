...
static bool mqtt_connect() {
  ...
  _tlsClient.setCACert(CA_CERT);
  if (strlen(CLIENT_CERT) > 10) {
    _tlsClient.setCertificate(CLIENT_CERT);
    _tlsClient.setPrivateKey(CLIENT_KEY);
  }
  _mqttClient.setServer(MQTT_HOST, MQTT_PORT);
  _mqttClient.setBufferSize(16384);  // 6400 bytes raw → ~8537 b64 → fits in 16 kB
...
static void publish_acoustic(float rms_g, float snr_db) {
  ...
  int n = snprintf(_jsonbuf, sizeof(_jsonbuf),
    "{"
      ...
      "\"pcm_b64\":\"%s\","
      "\"edge_snr_db\":%.2f,"
      "\"edge_vad_confidence\":%.4f,"
      "\"firmware_version\":\"%s\""
    "}",
    SENSOR_ID, SITE_ID, ts,
    SAMPLE_RATE, FRAME_SAMPLES,
    _b64buf, snr_db, vad_conf, FIRMWARE_VERSION
  );

  if (n >= (int)sizeof(_jsonbuf))
    Serial.println("[JSON] acoustic truncated!");
...
void setup() {
  ...
  snprintf(_acousticTopic,  sizeof(_acousticTopic),
           "omni/sensor/%s/%s/acoustic",  SITE_ID, SENSOR_ID);
