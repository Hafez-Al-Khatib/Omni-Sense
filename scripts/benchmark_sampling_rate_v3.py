#!/usr/bin/env python3
"""Benchmark IEP2 inference at 16 kHz vs 3.2 kHz resampled (proper proba extraction)."""
...
no_leak_idx = next(
    (int(k) for k, v in label_meta.items() if k != "_decision_threshold" and ("No_Leak" in v or "Normal" in v)),
    1,
)
leak_idx = 1 - no_leak_idx
print(f"Using production threshold: {THRESHOLD:.3f}")
print(f"Class mapping: {leak_idx}=Leak, {no_leak_idx}=No_Leak\n")
...
for f in test_files:
    ...
    # OOD scores (proper extraction)
    res_if_16k = sess_if.run(None, {"float_input": feat_16k[:, :39].astype(np.float32)})
    res_if_3k2 = sess_if.run(None, {"float_input": feat_3k2[:, :39].astype(np.float32)})
    if_16k = float(res_if_16k[1].flatten()[0])
    if_3k2 = float(res_if_3k2[1].flatten()[0])

    # Classification probabilities (proper extraction: result[1][0])
    res_xgb_16k = sess_xgb.run(None, {"float_input": feat_16k.astype(np.float32)})
    res_xgb_3k2 = sess_xgb.run(None, {"float_input": feat_3k2.astype(np.float32)})
    
    # Extract probability array (ONNX output may be dict or list)
    _p16 = res_xgb_16k[1][0]
    _p32 = res_xgb_3k2[1][0]
    proba_16k = np.array(list(_p16.values()), dtype=np.float32) if isinstance(_p16, dict) else np.array(_p16, dtype=np.float32)
    proba_3k2 = np.array(list(_p32.values()), dtype=np.float32) if isinstance(_p32, dict) else np.array(_p32, dtype=np.float32)
    
    # Apply production threshold
    pred_16k = "No_Leak" if proba_16k[no_leak_idx] >= THRESHOLD else "Leak"
    pred_3k2 = "No_Leak" if proba_3k2[no_leak_idx] >= THRESHOLD else "Leak"
    
    conf_16k = float(proba_16k[no_leak_idx if pred_16k == "No_Leak" else leak_idx])
    conf_3k2 = float(proba_3k2[no_leak_idx if pred_3k2 == "No_Leak" else leak_idx])
    ...
    print(
        f"{f.name:<55} {true_label:<10} {pred_16k:<10} {conf_16k:.3f}      "
        f"{pred_3k2:<10} {conf_3k2:.3f}      {if_16k:.3f}/{ood_16k:<4} {if_3k2:.3f}/{ood_3k2:<4}"
    )
